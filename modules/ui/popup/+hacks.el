...
;;;###package help-mode
(after! help-mode
  (defun enfer--switch-from-popup (location)
...
;;;###package profiler
(defun enfer*profiler-report-find-entry-in-other-window (orig-fn function)
  (cl-letf (((symbol-function 'find-function)
             (symbol-function 'find-function-other-window)))
    (funcall orig-fn function)))
(advice-add #'profiler-report-find-entry :around #'enfer*profiler-report-find-entry-in-other-window)
...
(progn
  ...
  (defun enfer*ignore-window-parameters (orig-fn &rest args)
    ...
    (cl-letf (((symbol-function #'windmove-find-other-window)
      ...
      (apply orig-fn args)))
  (advice-add #'windmove-up :around #'enfer*ignore-window-parameters)
  (advice-add #'windmove-down :around #'enfer*ignore-window-parameters)
  (advice-add #'windmove-left :around #'enfer*ignore-window-parameters)
  (advice-add #'windmove-right :around #'enfer*ignore-window-parameters))
