...
(defun +lookup--run-handlers (handler identifier origin)
  (enfer-log "Looking up '%s' with '%s'" identifier handler)
  (condition-case-unless-debug e
      (let ((wconf (current-window-configuration))
            (result (condition-case-unless-debug e
                        (+lookup--run-handler handler identifier)
                      (error
                       (enfer-log "Lookup handler %S threw an error: %s" handler e)
                       'fail))))
        (cond ((eq result 'fail)
                            ...
                            (let ((default-directory dir))
                              (without-project-cache!
                               (let ((file (projectile-completing-read "Find file: "
                                                                       ...
                                                                       :initial-input path)))
                                 (find-file (expand-file-name file (enfer-project-root)))
                                 (run-hooks 'projectile-find-file-hook))))))
                         (#'enfer-project-browse))))
             (find-file-at-point path))))))
