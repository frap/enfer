...
(defvar +snippets-dir (expand-file-name "snippets/" enfer-private-dir)
...
(def-package! yasnippet
  ...
  :config
  (setq yas-verbosity (if enfer-debug-mode 3 0)
  ...
  ;; Allow private snippets in ENFERDIR/snippets
  ...
  (add-hook 'enfer-project-hook #'+snippets|enable-project-modes)

  ;; Exit snippets on ESC from normal mode
  (add-hook 'enfer-escape-hook #'yas-abort-snippet)

  (after! smartparens
    ;; tell smartparens overlays not to interfere with yasnippet keybinds
    (advice-add #'yas-expand :before #'sp-remove-active-pair-overlay))

  ;; Enable `read-only-mode' for built-in snippets (in `enfer-local-dir')
...
(setq aya-persist-snippets-dir (concat enfer-etc-dir "auto-snippets/"))
