...
(defun +snippets--remove-p (x y)
  (and (equal (yas--template-key x) (yas--template-key y))
       (file-in-directory-p (yas--template-get-file x) enfer-emacs-dir)))
...
There are two groups of snippets in Enfer Emacs. The built in ones (under
`enfer-emacs-dir'; provided by Enfer or its plugins) or your private snippets
(outside of `enfer-eamcs-dir').
...
(defun +snippets/find ()
  ...
  (interactive)
  (let* ((dirs (enfer-files-in (cl-loop for dir in yas-snippet-dirs
                                       if (symbolp dir)
                                       collect (symbol-value dir)
                                       else collect dir)
                              :depth 0 :type 'dirs))
         (files (enfer-files-in dirs :depth 0 :full t)))
...
(defun +snippets/find-private ()
  ...
  (interactive)
  (enfer-project-find-file +snippets-dir))
...
(defun +snippets|read-only-maybe ()
  ...
  (when (file-in-directory-p default-directory enfer-local-dir)
    (read-only-mode 1)
