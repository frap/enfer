...
;;
;; + `bin/enfer install`: Installs packages that are wanted, but not installed.
;; + `bin/enfer update`: Updates packages that are out-of-date.
;; + `bin/enfer autoremove`: Uninstalls packages that are no longer needed.
;;
;; This system reads packages.el files located in each activated module (and one
;; in `enfer-core-dir'). These contain `package!' blocks that tell ENFER what
...
(defvar enfer-packages ()
  "A list of enabled packages. Each element is a sublist, whose CAR is the
package's name as a symbol, and whose CDR is the plist supplied to its
`package!' declaration. Set by `enfer-initialize-packages'.")

(defvar enfer-core-packages
...
(setq package--init-file-ensured t
      ...
      package-gnupghome-dir (expand-file-name "gpg" enfer-packages-dir)
...
;;; quelpa
(setq quelpa-dir (expand-file-name "quelpa" enfer-packages-dir)
      quelpa-verbose enfer-debug-mode
...
(defun enfer-initialize-packages (&optional force-p)
  "Ensures that Enfer's package management system, package.el and quelpa are
initialized, and `enfer-packages', `packages-alist' and `quelpa-cache' are
populated, if they aren't already.

If FORCE-P is non-nil, do it anyway.
If FORCE-P is 'internal, only (re)populate `enfer-packages'.

Use this before any of package.el, quelpa or Enfer's package management's API to
...
them."
  (let ((load-prefer-newer t)) ; reduce stale code issues
    ...
    ;; `enfer-packages' pass 'internal as FORCE-P or use `enfer/reload-packages'.
    (unless (eq force-p 'internal)
      ;; `package-alist'
      (when (or force-p (not (bound-and-true-p package-alist)))
        (enfer-ensure-packages-initialized 'force)
    ...
    ;; `enfer-packages'
    (when (or force-p (not enfer-packages))
      (setq enfer-packages (enfer-package-list)))))


;;
;;; Package API

(defun enfer-ensure-packages-initialized (&optional force-p)
    ...
    (let (byte-compile-warnings)
      (condition-case _
        ...
        ('error (package-refresh-contents)
                (setq enfer--refreshed-p t)
                (package-initialize))))))

(defun enfer-ensure-core-packages ()
  "Make sure `enfer-core-packages' are installed."
  (when-let (core-packages (cl-remove-if #'package-installed-p enfer-core-packages))
    (message "Installing core packages")
    (unless enfer--refreshed-p
      (package-refresh-contents))
    (dolist (package core-packages)
      (let ((inhibit-message t))
        (package-install package))
      (if (package-installed-p package)
          (message "✓ Installée %s" package)
        (error "✕ Ne pouvait pas installer %s" package)))
    (message "Installation de packages de base...fini")))
...
populate `enfer-packages' with metadata about the packages Enfer needs to keep
track of.
...
elsewhere."
  ...
  (enfer--assert-stage-p 'packages #'package!)
  (let ((old-plist (cdr (assq name enfer-packages))))
    (when recipe
    ...
    (let ((module-list (plist-get old-plist :modules))
          (module (or enfer--current-module
                      (let ((file (FILE!)))
    ...
    (when built-in
      (enfer-log "Ignoring built-in package '%s'" name)
    ...
    (macroexp-progn
     (append (when pin
               (enfer-log "Pinning package '%s' to '%s'" name pin)
               `((setf (alist-get ',name package-pinned-packages) ,pin)))
             `((setf (alist-get ',name enfer-packages) ',plist))
             (when disable
               (enfer-log "Désactiver le paquet '%s'" name)
               `((add-to-list 'enfer-disabled-packages ',name nil 'eq)
                 nil))))))
...
Only use this macro in a module's packages.el file."
  (enfer--assert-stage-p 'packages #'packages!)
  (macroexp-progn
   (cl-loop for desc in packages
            collect (macroexpand `(package! ,@(enfer-enlist desc))))))
...
Only use this macro in a module's packages.el file."
  (enfer--assert-stage-p 'packages #'disable-packages!)
  (macroexp-progn
