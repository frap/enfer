...
(describe "core/lib"
  ;; --- Helpers ----------------------------
  (describe "enfer-unquote"
    (it "unquotes a quoted form"
      (expect (enfer-unquote '(quote hello)) :to-be 'hello))
    (it "unquotes nested-quoted forms"
      (expect (enfer-unquote '(quote (quote (a b c)))) :to-equal '(a b c)))
    (it "unquotes function-quoted forms"
      (expect (enfer-unquote '(function a)) :to-be 'a))
    (it "does nothing to unquoted forms"
      (expect (enfer-unquote 'hello) :to-be 'hello)))

  (describe "enfer-enlist"
    (it "creates a list out of non-lists"
      (expect (enfer-enlist 'a) :to-equal '(a)))
