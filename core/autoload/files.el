  ...
  (cond
   ((listp path-or-paths)
    (cl-loop for path in path-or-paths
             ...
             nconc (apply #'enfer-files-in path (plist-put rest :relative-to relative-to))))
          ...
          (unless (member file '("." ".."))
            (let ((fullpath (expand-file-name file path)))
              (cond ((file-directory-p fullpath)
                     ...
                     (unless (< depth 1)
                       (setq result
                             (nconc result (apply #'enfer-files-in fullpath
...
(defun enfer--forget-file (old-path &optional new-path)
  ...
  (when (and (bound-and-true-p projectile-mode)
             (enfer-project-p)
             (projectile-file-cached-p old-path (enfer-project-root)))
    (projectile-purge-file-from-cache old-path))
  (when (bound-and-true-p save-place-mode)
    (save-place-forget-unreadable-files)))

(defun enfer--update-file (path)
  (when (featurep 'vc)
...
(defun enfer--copy-file (old-path new-path &optional force-p)
  (let* ((new-path (expand-file-name new-path))
         ...
         (project-root (enfer-project-root))
...
;;;###autoload
(defun enfer/delete-this-file (&optional path force-p)
  ...
  (let* ((fbase (file-name-sans-extension (file-name-nondirectory path)))
    ...
    (cond ((not (file-exists-p path))
          ...
          ((unwind-protect
             ...
             (let ((short-path (file-relative-name path (enfer-project-root))))
               (if (file-exists-p path)
                   (error "Failed to delete %s" short-path)
                 ;; Ensures that windows displaying this buffer will be switched
                 ;; to real buffers (`enfer-real-buffer-p')
                 (enfer/kill-this-buffer-in-all-windows buf t)
                 (enfer--forget-file path)
                 (enfer--update-file path)
                 (message "Successfully deleted %s" short-path))))))))

;;;###autoload
(defun enfer/copy-this-file (new-path &optional force-p)
...
file if it exists, without confirmation."
  (interactive
   (list (read-file-name "Copy file to: ")
         current-prefix-arg))
  (pcase (catch 'status
           (when-let (dest (enfer--copy-file (buffer-file-name) new-path force-p))
             (enfer--update-file new-path)
...
;;;###autoload
(defun enfer/move-this-file (new-path &optional force-p)
...
file if it exists, without confirmation."
  ...
  (pcase (catch 'status
           (let ((old-path (buffer-file-name))
             ...
             (when-let (dest (enfer--copy-file old-path new-path force-p))
               (when (file-exists-p old-path)
                 (delete-file old-path))
               (kill-current-buffer)
               (enfer--forget-file old-path new-path)
               (enfer--update-file new-path)
               (find-file new-path)
