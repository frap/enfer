...
(def-package! projectile
  ...
  (push (abbreviate-file-name enfer-local-dir) projectile-globally-ignored-directories)

  (defun enfer*projectile-default-generic-command (orig-fn &rest args)
...
the command instead."
  ...
  (advice-add #'projectile-default-generic-command :around #'enfer*projectile-default-generic-command)
  ...
  (defun enfer|cleanup-project-cache ()
    "Purge projectile cache entries that:

a) have too many files (see `enfer-projectile-cache-limit'),
b) represent blacklisted directories that are too big, change too often or are
   private. (see `enfer-projectile-cache-blacklist'),
c) are not valid projectile projects."
    (when (bound-and-true-p projectile-projects-cache)
      (cl-loop with blacklist = (mapcar #'file-truename enfer-projectile-cache-blacklist)
               for proot in (hash-table-keys projectile-projects-cache)
               if (or (not (stringp proot))
                      (>= (length (gethash proot projectile-projects-cache))
                          enfer-projectile-cache-limit)
                      (member (substring proot 0 -1) blacklist)
                      (and enfer-projectile-cache-purge-non-projects
                           (not (enfer-project-p proot))))
               do (enfer-log "Removed %S from projectile cache" proot)
               and do (remhash proot projectile-projects-cache)
               and do (remhash proot projectile-projects-cache-time)
               and do (remhash proot projectile-project-type-cache))
      (projectile-serialize-cache)))
  (unless noninteractive
    (add-hook 'kill-emacs-hook #'enfer|cleanup-project-cache))
  ...
  (defun enfer*projectile-locate-dominating-file (orig-fn file name)
    "Don't traverse the file system if on a remote connection."
    (when (and (stringp file)
               (not (file-remote-p file nil t)))
      (funcall orig-fn file name)))
  (advice-add #'projectile-locate-dominating-file :around #'enfer*projectile-locate-dominating-file)
  ...
  (cond
   ...
   ((executable-find enfer-projectile-fd-binary)
    (setq projectile-git-command (concat
                                  enfer-projectile-fd-binary
...
A project can be enabled through .dir-locals.el too, by setting `enfer-project'.
  ...
  (let ((init-var (intern (format "%s-init" name))))
    `(progn
       ...
       (define-minor-mode ,name
         ...
         (if (not ,name)
             ,on-exit
           (run-hook-with-args 'enfer-project-hook ',name ,name)
           ,(when on-load
              `(unless ,init-var
                 ,on-load
                 (setq ,init-var t)))
