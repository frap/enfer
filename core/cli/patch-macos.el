...
(dispatcher! (patch-macos)
  (enfer-patch-macos (or (member "--undo" args)
                        (member "-u" args))
                    (enfer--find-emacsapp-path))
  "Patches Emacs.app to respect your shell environment.

WARNING: This command is deprecated. Use 'enfer env' instead.
...
(defun enfer--find-emacsapp-path ()
...
(defun enfer-patch-macos (undo-p appdir)
  "Patches Emacs.app to respect your shell environment."
  (unless IS-MAC
  ...
  (let ((oldbin (expand-file-name "Contents/MacOS/Emacs" appdir))
    ...
    (cond (undo-p
          ...
          ((or enfer-auto-accept
               (y-or-n-p
                (concat "(WARNING: patch-macos is deprecated, use `enfer env refresh` instead)\n\n"
                        "Enfer would like to patch your Emacs.app bundle so that it respects\n"
                        "your shell configuration. For more information on why and how, run\n\n"
                        "  bin/enfer help patch-macos\n\n"
                        "Patch Emacs.app?")))
